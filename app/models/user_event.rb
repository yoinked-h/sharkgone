  ...
  concerning :ConstructorMethods do
    class_methods do
      ...
      def build_from_request(user, category, request)
        ...
        user.user_events.build(user: user, category: category, ip_addr: ip_addr, session_id: request.session[:session_id], user_agent: request.user_agent, fingerprint: json, fingerprint_hash: hash)
      end
