...
class UserEvent < ApplicationRecord
  ...
  def self.search(params, current_user)
    q = search_attributes(params, [:id, :created_at, :updated_at, :category, :user, :ip_addr, :session_id, :user_agent, :metadata, :ip_geolocation, :fingerprint, :fingerprint_hash], current_user: current_user)
  ...
  concerning :ConstructorMethods do
    class_methods do
      ...
      def create_from_request!(user, category, request, login_session: nil)
        ...
        create!(user: user, category: category, ip_addr: ip_addr, login_session_id: login_session_id, session_id: request.session[:session_id], user_agent: request.user_agent, fingerprint: json, fingerprint_hash: hash)
      end
    end
  end
