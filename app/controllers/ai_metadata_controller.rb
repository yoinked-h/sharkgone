...
class AIMetadataController < ApplicationController
  ...
  def create_or_update
    post_id = params.dig(:post, :ai_metadata).delete(:post_id) || params[:post_id]
    @ai_metadata = authorize AIMetadata.find_or_initialize_by(post_id: post_id)
    if @ai_metadata.update(updater: CurrentUser.user, parameters: params.dig(:post, :ai_metadata))
      flash[:notice] = "AI metadata updated"
    end
